...
const createEmployeeIdProof = async (req, res) => {
    try {
        ...
        const isEmployeeExist = await db.query(`SELECT * from employee_profiles where employee_id = $1`, [employeeIdProofData.employee_id]);
        ...
        const employeeIdProofQuery = await db.query(
            `INSERT INTO employee_id_proofs (employee_id, proof_name, proof_number, id_proof_url, created_at, updated_at) VALUES ('${employeeIdProofData.employee_id}','${employeeIdProofData.proof_name}','${employeeIdProofData.proof_number}','${employeeIdProofData.id_proof_url}', '${now}', '${now}') RETURNING *`
        );
    ...
    try {
        ...
        if (Object.keys(filterQuery).length !== 0) {
            if (filterQuery.employee_id) {
                allIdProofs = allIdProofs.filter((e) => e.employee_id == filterQuery.employee_id);
            }
        }
        return res.status(RestAPI.STATUSCODE.ok).send({
            statusCode: RestAPI.STATUSCODE.ok,
...
const replaceEmployeeIdProof = async (req, res) => {
    try {
        ...
        const updateQuery = `UPDATE employee_id_proofs SET employee_id='${employeeIdProofData.employee_id}', proof_name='${employeeIdProofData.proof_name}', proof_number='${employeeIdProofData.proof_number}', id_proof_url='${employeeIdProofData.id_proof_url}', updated_at = '${now}' WHERE id='${req.params.id}'`;

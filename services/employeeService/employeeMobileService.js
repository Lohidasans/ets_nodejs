...
const createEmployeeMobileDetails = async (req, res) => {
    try {
        ...
        const isEmployeeExist = await db.query(`SELECT * from employee_profiles where employee_id = $1`, [employeeMobileDetailsData.employee_id]);
        ...
        const employeeMobileDetailsQuery = await db.query(
            `INSERT INTO employee_mobiles (employee_id,mobile_name, mobile_weight, mobile_front_image_url, mobile_back_image_url, mobile_description, created_at, updated_at) VALUES ('${employeeMobileDetailsData.employee_id}','${employeeMobileDetailsData.mobile_name}','${employeeMobileDetailsData.mobile_weight}','${employeeMobileDetailsData.mobile_front_image_url}','${employeeMobileDetailsData.mobile_back_image_url}','${employeeMobileDetailsData.mobile_description}', '${now}', '${now}') RETURNING *`
        );
...
const getAllEmployeeMobileDetails = async (req, res) => {
    try {
        var allEmployeeMobileDetails = await db.query(
            `SELECT * FROM employee_mobiles ORDER BY id`
        );
        ...
        if (Object.keys(filterQuery).length !== 0) {
            if (filterQuery.employee_id) {
                allMobileDetails = allMobileDetails.filter((e) => e.employee_id == filterQuery.employee_id);
            }
        }
        return res.status(RestAPI.STATUSCODE.ok).send({
            statusCode: RestAPI.STATUSCODE.ok,

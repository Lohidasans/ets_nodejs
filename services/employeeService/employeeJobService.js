...
const createEmployeeJob = async (req, res) => {
    try {
        ...
        const isEmployeeExist = await db.query(`SELECT * from employee_profiles where employee_id = $1`, [employeeJobData.employee_id]);
        ...
        const employeeJobQuery = await db.query(
            `INSERT INTO employee_jobs (employee_id,organization, from_date, to_date, description, certificate_url, created_at, updated_at) VALUES ('${employeeJobData.employee_id}','${employeeJobData.organization}','${employeeJobData.from_date}','${employeeJobData.to_date}','${employeeJobData.description}','${employeeJobData.certificate_url}', '${now}', '${now}') RETURNING *`
        );
...
const getAllEmployeeJob = async (req, res) => {
    try {
        var allEmployeeJobs = await db.query(
            `SELECT * FROM employee_jobs ORDER BY id`
        );
        ...
        if (Object.keys(filterQuery).length !== 0) {
            if (filterQuery.employee_id) {
                allJobs = allJobs.filter((e) => e.employee_id == filterQuery.employee_id);
            }
        }
        return res.status(RestAPI.STATUSCODE.ok).send({
            statusCode: RestAPI.STATUSCODE.ok,

...
const createEmployeeOrnamentDetails = async (req, res) => {
    try {
        ...
        const isEmployeeExist = await db.query(`SELECT * from employee_profiles where employee_id = $1`, [employeeOrnamentDetailsData.employee_id]);
        ...
        const employeeOrnamentDetailsQuery = await db.query(
            `INSERT INTO employee_ornaments (employee_id,ornament_name, ornament_weight, ornament_image_url, ornament_description, created_at, updated_at) VALUES ('${employeeOrnamentDetailsData.employee_id}','${employeeOrnamentDetailsData.ornament_name}','${employeeOrnamentDetailsData.ornament_weight}','${employeeOrnamentDetailsData.ornament_image_url}','${employeeOrnamentDetailsData.ornament_description}', '${now}', '${now}') RETURNING *`
        );
...
const getEmployeeOrnamentDetailsById = async (req, res) => {
    try {
        const isEmployeeOrnamentDetailsExist = await db.query(
            `SELECT * FROM employee_ornaments WHERE id = $1`,
            [req.params.id]
        );
...
const getAllEmployeeOrnamentDetails = async (req, res) => {
    try {
        var allEmployeeOrnamentDetails = await db.query(
            `SELECT * FROM employee_ornaments ORDER BY id`
        );
        ...
        if (Object.keys(filterQuery).length !== 0) {
            if (filterQuery.employee_id) {
                allOrnamentDetails = allOrnamentDetails.filter((e) => e.employee_id == filterQuery.employee_id);
            }
        }
        return res.status(RestAPI.STATUSCODE.ok).send({
            statusCode: RestAPI.STATUSCODE.ok,

...
const getEmployeeSecurityDetails = async () => {
    const getEmoployeeData = await db.query(`SELECT     
            ...
            security_managements.date AS in_date,
            security_managements.time AS in_time
            ...
            WHERE DATE(security_managements.date) =  CURRENT_DATE
            GROUP BY 
                employee_profiles.employee_id,
                employee_profiles.name,
                employee_profiles.image_url,
                sub_teams.sub_team,
                security_managements.date,
                security_managements.time,
                users.name
                `);
